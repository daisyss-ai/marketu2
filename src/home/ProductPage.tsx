...
function formatProductType(type: ProductDetail['type']): string {
  ...
  if (type === 'physical_product') return 'Produto físico';
  return 'Serviço';
}
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      ...
      const { data: productRow } = await supabase
        .from('products').select('category_id').eq('id', product.id).maybeSingle();
      ...
      const { data } = await supabase
        ...
        .eq('is_active', true).eq('category_id', productRow.category_id).neq('id', product.id).limit(5);
      if (data) {
        setRelatedProducts(data.map((p: any) => ({
          id: p.id, title: p.title, price: Number(p.price ?? 0), seller: 'MarketU',
          img: (p.product_media as any[])?.find((m: any) => m.is_preview)?.url || (p.product_media as any[])?.[0]?.url || null,
          category: (p.categories as any)?.name || 'Geral', statusColor: 'bg-green-400', rating: null, total_reviews: 0,
        })));
      }
    };
    fetchRelated();
  ...
  useEffect(() => {
    const fetchSellerProducts = async () => {
      ...
      const { data } = await supabase
        .from('products')
        .select('id, title, price, is_free, seller_id, categories(name), product_media(url, is_preview, position)')
        .eq('is_active', true).eq('seller_id', product.sellerId).neq('id', product.id).limit(5);
      if (data) {
            ...
            <div className="mt-4 grid grid-cols-4 gap-3">
              {galleryImages.map((src, index) => {
                ...
                return (
                  ...
                  >
                    <Image src={src} alt={`${product.title} ${index + 1}`} width={160} height={160} className="h-full w-full object-cover" />
                  </button>
                );
              })}
            </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="mb-1 flex items-baseline gap-2">
                <div className="text-3xl font-extrabold text-[#4B187C]">{formatCurrency(product.price)}</div>
              </div>
              ...
              <div className="mt-2 flex items-center gap-2 text-xs text-gray-600">
                ...
                <span>({product.totalReviews} avaliações)</span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-purple-100 bg-white p-5 shadow-sm">
              ...
              <InterestButton productId={product.id} sellerId={product.sellerId} currentUserId={currentUserId} className="mt-4" />
            </div>
            ...
            <div className="grid gap-4 text-sm md:grid-cols-2">
              <div className="space-y-2 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                <div className="flex justify-between gap-4">
                  ...
                  <span className="text-right font-medium text-gray-900">{formatProductType(product.type)}</span>
                </div>
                <div className="flex justify-between gap-4">
                  <span className="text-gray-500">Preço</span>
                  <span className="text-right font-medium text-gray-900">{formatCurrency(product.price)}</span>
                </div>
                <div className="flex justify-between gap-4">
                  <span className="text-gray-500">Publicado</span>
                  <span className="text-right font-medium text-gray-900">{formatCreatedAt(product.createdAt)}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col justify-between rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  {product.sellerAvatarUrl ? (
                    <Image src={product.sellerAvatarUrl} alt={product.sellerName} width={40} height={40} className="h-10 w-10 rounded-full object-cover" />
                  ) : (
                    <div className="h-10 w-10 rounded-full bg-pink-200" />
                  )}
                  <div>
                    <Link href={`/profile/user/${product.sellerId}`} className="text-sm font-semibold text-gray-900 hover:underline">
                      {product.sellerName}
                    </Link>
                    <div className="flex items-center gap-1 text-xs text-gray-500">
                      <Star className="h-3 w-3 text-yellow-400" />
                      <span>{product.rating?.toFixed(1) ?? 'Sem avaliações'} ({product.totalReviews})</span>
                    </div>
                  </div>
                </div>
            ...
            <ContactSellerButton productId={product.id} sellerId={product.sellerId} currentUserId={currentUserId} />
          </div>
        </div>
      </main>
